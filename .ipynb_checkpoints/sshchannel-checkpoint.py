    def select_channel(self, host):
        user='lym';password="8566231jkl;'"
        port=22
    ...
    def sendcommand(self, channel, out, command='sinfo'):
    ...
    def upload(self, channel, local_path, server_path):
        node=self.sendcommand(self.channel, out=0, command='echo $HOSTNAME')[0].strip('\n')
        channel = self.channel
        ftp = self.ftp
        dirlist = ftp.listdir(".")
    ...
    def download(self, channel, server_path, local_path):
        node=self.sendcommand(self.channel, out=0, command='echo $HOSTNAME')[0].strip('\n')
        channel = self.channel
        ftp = self.ftp
        dirlist = ftp.listdir(".")
