...
class niceday:
    ...
    def __init__(self, node, PJname, PJdir):
        ...
        if node != 'jupyter':
            ...
            self.channel, self.ftp = select_channel(self.node)
        ...
        if node == 'jupyter':
            ...
            try:
                ...
                self.channel, self.ftp = select_channel(self.previous_working_node)
    ...
    def mkdir(self):  # 20220608
        ...
        try:
            ...
            sendcommand(channel, 0, command=command)
        ...
        # ## create input file
        try:
            r = open(self.L_input + self.inputfile, 'r')
            r.close()
        except:
        ...
        if str(nb_path) != str(target + '.ipynb'):
            if not os.path.isfile(target + '.ipynb'):
                with open(target + '.txt', 'w') as handle:
                    handle.write(
                        str(self.current_login_Node(out=0)[0]) + '\t' + str(self.current_login_Node(out=0)[1]) + '\n')
    ...
    # #### process #####
    def checkinput(self, out=1, filter='all',local=False):
        ftp = self.ftp
        inputfile = self.inputfile
        R_workdir = self.R_workdir

        if local:
            print('looking locally in ', self.L_input + inputfile)
            F = open(self.L_input + inputfile)
            f = F.readlines()
        else:
            print('looking remotely in ', R_workdir + inputfile)
    ...
    def totalsteps(self,checklocal=False):
        totalsteps = 0
        if checklocal:
            for i in self.checkinput(out=0, filter='run',local=True):
                totalsteps += int(i[-1])
        else:
    ...
    def checklog(self, refresh=1, Lastlog=0, Check_lammpslog=1, Check_slurmlog=1, Download_log=1, checklog_local=1):
        ...
        ftp = self.ftp
        nodes, tempdir = self.nodes_reader()
        ...
        if refresh == 1:
            if checklog_local == 1 or Download_log == 1:
                if self.node == 'jupyter':
                    ...
                    download(channel, ftp, R_workdir + slurmerrorlog, L_log + slurmerrorlog)
                    download(channel, ftp, R_workdir + slurmlog, L_log + slurmlog)
                    # try:
                    tmp_workdir = tempdir + 'lym@' + self.current_job_id() + '/'

                    if 'null' not in tmp_workdir:
                        download(channel, ftp, tmp_workdir + inputfile, L_log + inputfile + '_remote')
                        download(channel, ftp, tmp_workdir + lammpslog, L_log + lammpslog)
                        download(channel, ftp, tmp_workdir + inputfile + '.out', L_log + inputfile + '.out')

                    else:
                        try:
                            tmp_workdir = self.previous_working_dir
                            print('downloading from previous_working_dir')
                            output = download(channel, ftp, tmp_workdir + inputfile, L_log + inputfile + '_remote')
                        except:
                        ...
                        if 'failed' in output:
                            ...
                            download(channel, ftp, R_workdir + inputfile, L_log + inputfile + '_remote')
                            download(channel, ftp, R_workdir + lammpslog, L_log + lammpslog)
                            download(channel, ftp, R_workdir + inputfile + '.out', L_log + inputfile + '.out')
                        else:
                            download(channel, ftp, tmp_workdir + lammpslog, L_log + lammpslog)
                            download(channel, ftp, tmp_workdir + inputfile + '.out', L_log + inputfile + '.out')
                else:
        ...
        def check_log(logfile):
            ...
            if len(f) > 0:
                ...
                print('no log in selected log file',logfile)
    ...
    def plotlog(self, logfile, refresh=1, plot=True, a=0, b=-1, skip=1, plotlog_local=1):
        ...
        if plotlog_local == 1:
            print('Read local file:',L_log + logfile)
            with open(L_log + logfile, encoding="utf8", errors='ignore') as log:
                lmplog = log.readlines()
            print(L_log + logfile)
            totalsteps = self.totalsteps(checklocal=True)  # 并不完全准确？
        else:
            log = ftp.open(R_workdir + logfile)
            lmplog = log.readlines()
            totalsteps = self.totalsteps(checklocal=False)  # 并不完全准确？
        ...
        try:
            ...
            taillammpslog = sendcommand(channel, out=0, command='tail ' + R_workdir + lammpslog)
            jobsucceed = False
            ...
            if bar_number > 50:
                ...
                progress = 0
                print(
                    f'Are you run lammps with restart_data? The detected currentstep / totalsteps are {currentstep} and {totalsteps}, respectively')
            else:
                progressbar = list('|' + '-' * 50 + '|')
                for i in range(bar_number):
                    progressbar[i + 1] = '|'
                print('Progress:',
                      progress + ''.join(progressbar) + ' ' + str(currentstep) + '/' + str(totalsteps) + '  |' + str(
                          round(predicted_time, 2)) + ' hours left')
            ...
            if plot:
                c_reaxdict = {'v_eb': 'bond energy', 'v_ea': 'atom energy', 'v_elp': 'lone-pair energy',
                              ...
                              'v_ecoa': 'valence angle conjugation energy', 'v_ehb': 'hydrogen bond energy',
                              'v_et': 'torsion energy', 'v_eco': 'conjugation energy', 'v_ew': 'van der Waals energy',
                              'v_ep': 'Coulomb energy', 'v_efi': 'electric field energy (always 0.0)',
                ...
                for x_picked_col in [0, 1]:  # xaxis数据源
                    ...
                    for y_picked_col in range(0, len(float_result)):
                        if x_picked_col != y_picked_col:
                            ...
                            print('skipping meaningless plot: ' + str(title[y_picked_col]) + ' vs. ' + str(
                                title[y_picked_col]))
                    plt.show
        except:
            print('plot log failed, is there any data in 0Thermodata_log.txt?')

    def results_checker(self, ranges, columns=None, plot=False):  # 20220612
        if columns is None:
            columns = [0, 1, 5, 6]
    ...
    def trajviewer(self, refresh=1, viewangle='Half', write='pdb', frame=None, info=True, atom_types=None,
                   trj_format='LAMMPS'):
        ...
        a_trj = L_traj + trajs
        lammpstrj = self.lammpstrj
        a_lammpstrj = L_traj + lammpstrj
        nodes, tempdir = self.nodes_reader()
        ...
        if trj_format == 'LAMMPSDUMP':
            print(a_lammpstrj)
            trajs_type = lammpstrj
        elif trj_format == 'LAMMPS':
            trajs_type = trajs
        else:
            trajs_type = trajs

        if refresh != 1:
            if not os.path.isfile(L_traj + trajs_type):
                download(channel, ftp, R_workdir + trajs_type, L_traj + trajs_type)
            if not os.path.isfile(L_traj + getlmpdatafile):
                download(channel, ftp, R_workdir + getlmpdatafile, L_traj + getlmpdatafile)
            if info:
                print('-----------------------------------------------------------------------------------------')
                ...
                print(sendcommand(channel, 0, 'ls -lh ' + R_workdir + trajs_type))
                print('--------------------------  version of local files--------------------------------')
                print(self.listlocallogfile(L_traj + trajs_type))
        ...
        if info:
            print('topo|trajs: ', getlmpdatafile, '|', L_traj + trajs_type)

        a_lengthunit = "A"
        a_timeunit = "fs"

        if trj_format == 'LAMMPS':
        ...
        elif trj_format == 'LAMMPSDUMP':
            try:
                topo_format = 'DATA'
                u = mda.Universe(a_topo, a_lammpstrj, topology_format=topo_format, format=trj_format,
                                 lengthunit=a_lengthunit, timeunit=a_timeunit)
            except:
                print('are you using reaxff filed? loading trj as lammpsdump format')
                u = mda.Universe(a_lammpstrj, format=trj_format)
        if info:
            print(u.kwargs)
        detected_atom_types = sorted(list(set(u.atoms.types)))
        print('atom types in the system:', detected_atom_types)
        ...
        if atom_types is None:
            print('Obtaining atom_types from input file')
            with open(L_workdir + '#input/' + inputfile, 'r') as lmpreader:
                lmpdata = lmpreader.readlines()
                for line in lmpdata:
                    if 'pair_coeff' in line:
                        print(line)
                        atom_types = line.split('.ffield')[-1].split()

        def get_capitalized_letters(start_idx, end_idx):
            """Returns a list of capitalized letters from start_idx to end_idx (inclusive)."""
            return [chr(ord('A') + idx) for idx in range(start_idx, end_idx + 1)]

        chainIDs = get_capitalized_letters(0, len(atom_types) - 1)

        # for i in range(len(atom_types)):
        for idx, i in enumerate(detected_atom_types):
            u.select_atoms('type  ' + str(i)).names = [atom_types[idx]] * len(u.select_atoms('type  ' + str(i)))
            u.select_atoms('type  ' + str(i)).chainIDs = chainIDs[idx]

        # indexing
        index = []
        # for i in range(len(atom_types)):
        for i in detected_atom_types:
            index.append(u.select_atoms(f'type  {str(i)}').indices)
    ...
    def cut_a_singleframe(self, u, frame, topo, atom_types, chainIDs, topo_format='DATA'):
        ...
        trj = '/tmp/temp.dcd'
        usingle = self.import_to_mda(topo, trj, atom_types, chainIDs, dt=0, topo_format=topo_format,
                                     trj_format='LAMMPS',
    ...
    def current_login_Node(self, out):
        ...
        k = sendcommand(channel, 0, 'uname -a')
        results0 = k[0].split()
        d = sendcommand(channel, 0, 'date')
        results = d[0].split()
    ...
    def times_job_elapsed(self):
        taillammpslog = sendcommand(self.channel, out=0, command='tail ' + self.R_workdir + self.lammpslog)
        jobsucceed = False
        elapsedtime = 0
        ...
        if not jobsucceed:
            ...
            jobids, jobnames, jobstat, elapsedtimes, nodelists = [[] for i in range(5)]
            datas = sendcommand(self.channel, out=0, command='export SQUEUE_FORMAT="%.18i %.15P %.50j %.8u %.8T %.10M %.9l %.6D %R" && squeue')
            for i in datas[1:]:
                keys=i.split()
                jobids.append(keys[0])
                jobnames.append(keys[1])
                jobstat.append(keys[4])
                elapsedtimes.append(keys[5])
                nodelists.append(keys[-1])
            # jobids, jobnames, jobstat, elapsedtimes, nodelists = \
            #     sendcommand(self.channel, out=0, command='squeue')[-1]
            ...
            if self.node == 'jupyter':
                currentnode = self.current_working_node(allnodes=True)
            else:
                currentnode = self.current_login_Node(out=0)[0]

            if currentnode != 'null':
                for i in range(len(nodelists)):
                    if nodelists[i] == currentnode and jobstat[i] == 'RUNNING':
                        print('Current node info: ', jobids[i], jobnames[i], str(jobstat[i]), elapsedtimes[i],
                              nodelists[i])
    ...
    def scancel(self, x):
        channel = self.channel
        All_jobsID = sendcommand(channel, 0, 'squeue')[0]
        PD_jobsID = sendcommand(channel, 0, 'squeue')[1]
        R_jobsID = sendcommand(channel, 0, 'squeue')[2]
        if x == 'all':
            jobsID = All_jobsID
            end = len(jobsID)
            start = 0
            for i in range(start, end):
                sendcommand(channel, 0, 'scancel ' + jobsID[i])
        elif x == 'pending':
            jobsID = PD_jobsID
            end = len(jobsID)
            start = 0
            ...
            for i in All_jobsID:
                if x == str(i):
                    sendcommand(channel, 0, 'scancel ' + str(x))
                    rightinput = True
                    break
            if not rightinput:
    ...
    def sinfo(self):
        channel = self.channel
        sendcommand(channel, 1, 'sinfo')

    def squeue(self, out=1):
        channel = self.channel
        results = sendcommand(channel, out, 'squeue --format="%.18i %.15P %.50j %.8u %.8T %.10M %.9l %.6D %R"')
        return results

    def current_working_node(self, allnodes=False):
        ...
        for index, name in enumerate(names):
            if 'in.' + self.PJname == name:
                ...
                if not allnodes:
                    if "[" in node_for_job:
                        print('multi nodes are used in this job, will chose one of the node as login site:',node_for_job)
                        node_for_job = 'node'+node_for_job.split('[')[-1].split('-')[-1].split(',')[-1].split(']')[0]
                return node_for_job

        if node_for_job == 'null':
    ...
    def nodes_reader(self):
        try:
            with open(self.L_input+'sbatch.sh','r') as sh_reader:
                lines=sh_reader.readlines()
                nodes = 1
                for l in lines:
                    if "--nodes" in l:
                        nodes = int(l.split('=')[-1])
        except:
            print('warning! not found sbatch.sh in local input folder')
            nodes = 1
        if nodes == 1:
            tempdir = '/tmp/'
        else:
            tempdir = '/home/lym/tmp/'
        return nodes, tempdir
        ...
        try:
            ...
            if out == 1:
                ...
                if current_working_node != 'null':
                    print('tmp_working_dir: ', tempdir + 'lym@' + current_job_id + '/')
                else:
                    print('job is not running, will not change tmp_working_dir!')
            ...
            if current_working_node != 'null':
                ...
                with open(target + '.txt', 'w') as W:
                    ...
                    W.write('tmp_working_dir: ' + tempdir + 'lym@' + current_job_id + '/' + '\n')


                if tempdir + 'lym@' + current_job_id + '/' != self.previous_working_dir:
                    self.previous_working_node = current_working_node
                    self.previous_working_dir = tempdir + 'lym@' + current_job_id + '/'
                    if out == 1:
            ...
            try:
                if str(ids[i]) == submited_job_id:
                    ...
                    if 'RUNNI' in stats[i]:
                        ...
                        print('Loging previous_working_info')
                        self.refresh_node_info()
                        self.previous_working_node, self.previous_working_dir = self.previous_working_info()
                    if 'P' in stats[i]:
                        print('Job is in stat:', stats[i])
                        print("Warning, you've submited a PD job..")
                        print('Warning! Pending job detected, will not log previous_working_info')
                        # self.refresh_node_info()
                    print('--------------------------------------------------------')
            except UnboundLocalError:
                print('怎么搞的呢,你这个糊涂蛋,作业提交失败了,先检查你的sbatch脚本\n'*3)
                return ('搞不动就先别学了,去玩会换个脑子')
    ...
    def previous_working_info(self):
        ...
        target = self.L_workdir + self.PJname + '_' + self.node
        # print('Reading form job info file')
        try:
            print("Updating landing location form ship's log")
            ...
            print("It should be our first sail. OTHERWISE, Ship's log must has been stolen by aliens!")

    # htop ------------------------------------------------------------------------------------------------------------
    def top(self, out=1):
        channel = self.channel
        sendcommand(channel, 0, command='top -b -n1 > /tmp/top.out')
        toplog = sendcommand(channel, 0, command='head -c 380 /tmp/top.out  | tail -c +0')
        if out == 1:
            for i in toplog:
                print(i)
        return toplog
    ...
    def R_workdirlist(self, sort='date'):
        R_workdir = self.R_workdir
        channel = self.channel
        x = sendcommand(channel, 0, 'ls -lh ' + R_workdir)
        info = []
        for i in x:
            x = i.split()
    ...
    @staticmethod
    def import_to_mda(topo, trj, atom_types, chainIDs, dt=0, topo_format='DATA', trj_format='LAMMPS',
        ...
        if topo_format == 'DATA':
            ...
            u = mda.Universe(topo, trj, topology_format=topo_format, format=trj_format,
                             atom_style=a_atom_style, lengthunit=a_lengthunit, timeunit=a_timeunit, in_memory=in_memory,
    ...
    @staticmethod
    def nv_add_representation(vw, atom_indexs, viewangle='Half'):
        """
        ...
        vw.add_representation('spacefill', selection=':A' + atom_selections, color='silver',
                              radius='0.8')  # atom name: .CA, .C, .N  element name: _H, _C, _O, chain name: :A
        vw.add_representation('spacefill', selection=':B' + atom_selections, color='red',
                              radius='0.8')  # atom name: .CA, .C, .N  element name: _H, _C, _O, chain name: :A
        vw.add_representation('spacefill', selection=':C' + atom_selections, color='black',
                              radius='0.8')  # atom name: .CA, .C, .N  element name: _H, _C, _O, chain name: :A
        vw.add_representation('spacefill', selection=':D' + atom_selections, color='yellow',
                              radius='0.8')  # atom name: .CA, .C, .N  element name: _H, _C, _O, chain name: :A
        vw.add_representation('spacefill', selection=':E' + atom_selections, color='grey',
                              radius='0.6')  # atom name: .CA, .C, .N  element name: _H, _C, _O, chain name: :A
        vw.add_representation('spacefill', selection=':F' + atom_selections, color='pink',
                              radius='0.8')  # atom name: .CA, .C, .N  element name: _H, _C, _O, chain name: :A
        T3 = time.time()
        vw.player.parameters = dict(delay=0.004, step=-1)
        # vw.background = 'black'
